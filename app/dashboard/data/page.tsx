...
import { Eye, Download, Trash2, AlertTriangle, FolderOpen } from 'lucide-react';
...
export default function DataManagementPage() {
  ...
  return (
    <DashboardShell
      ...
      subtitle="View, download, or delete your family's data"
    >
      <motion.div
      ...
      >
        <Card className={`mb-8 border-4 ${theme.border} ${isDark ? 'bg-blue-900/30' : 'bg-blue-50/50'} p-6`}>
          <div className="flex gap-4">
            <div className="text-4xl">🔒</div>
            <div>
              <h2 className={`mb-2 text-lg font-bold ${isDark ? 'text-blue-200' : 'text-blue-900'}`}>Your Privacy Matters</h2>
        ...
        <h2 className={`mb-6 text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Your Children&apos;s Data</h2>
        ...
        {isLoading ? (
          ...
          <Card className={`p-12 text-center border-4 ${theme.border} ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
            ...
            <h3 className={`mb-2 text-xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>No children added yet</h3>
            <p className={`mb-6 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>Add a child to start managing their data</p>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {children.map((child, i) => (
              <PremiumCard key={child.id} delay={i * 0.1} className={`p-6 border-4 ${theme.border}`}>
                <div className="mb-4 flex items-center gap-4">
                  <div className={`flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br ${theme.gradient} text-2xl font-bold text-white shadow-lg`}>
                    {child.name.charAt(0)}
                  </div>
                  <div>
                    <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{child.name}</h3>
                    <p className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{child.grade_level}</p>
